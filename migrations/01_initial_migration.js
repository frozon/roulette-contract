...
module.exports = async function(deployer, environment) {
  if (environment === 'development') {
    ...
    await deployer.deploy(daiMock, '1').then(async (daiToken) => {
      await deployer.deploy(cSphere).then(async (cSphereToken) => {
        ...
        console.log(`DAI_TOKEN_ADDRESS='${daiToken.address}'`)
        console.log(`BET_TOKEN_ADDRESS='${cSphereToken.address}'`);
